...
class ArbeScraper(BaseScraper):
    def __init__(self, url: str = "https://arberobotics.com/career/", headless: bool = True):
        # We inherit to keep the interface consistent, but we'll use the API.
        super().__init__(url, headless)
        self.company_uid = 'C6.001'
        self.token = '6C12886D821B0414432886144328862F472F47'
    ...
    def extract_jobs(self):
        """
        Extracts job titles and links from Arbe (Comeet) API.
        """
        jobs = []
        print(f"Fetching jobs from Arbe API...")
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"Error: Arbe API returned status {response.status_code}")
                return []
            ...
            print(f"Error fetching Arbe jobs: {e}")
