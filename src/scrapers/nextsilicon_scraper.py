...
class NextSiliconScraper(BaseScraper):
    def __init__(self, url: str = "https://www.nextsilicon.com/careers", headless: bool = True):
        # We inherit to keep the interface consistent, but we'll use the API.
        super().__init__(url, headless)
        self.company_uid = '18.007'
        self.token = '817308A2873102E1845308A308A40B8308A205C'
    ...
    def extract_jobs(self):
        """
        Extracts job titles and links from NextSilicon (Comeet) API.
        """
        jobs = []
        print(f"Fetching jobs from NextSilicon API...")
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"Error: NextSilicon API returned status {response.status_code}")
                return []
            ...
            print(f"Error fetching NextSilicon jobs: {e}")
